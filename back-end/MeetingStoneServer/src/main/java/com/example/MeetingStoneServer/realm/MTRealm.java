package com.example.MeetingStoneServer.realm;
...
public class MTRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        String userNum = authenticationToken.getPrincipal().toString();
        User user = userService.getByUsernum(userNum);
        String passwordInDB = user.getPassword();
        String salt = user.getSalt();
        return new SimpleAuthenticationInfo(userNum,passwordInDB, ByteSource.Util.bytes(salt),getName());
    }
}
